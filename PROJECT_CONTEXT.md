...
**Last updated:** 2026-06-05 (clean-restart corrections)
...
- **Tech stack:** Python (stdlib), PySide6 UI, SQLite database, Google Books API, SMTP via Gmail app password.
- **Current phase:** Clean restart. Code being written from scratch for full authorship. About to begin Phase 1 (foundation).
...
|---|---|---|
| Language | Python 3.x (confirm exact version on dev machine — currently noted as 3.13 in IA draft) | Mature stdlib, broad library support, IB-familiar |
...
StudentProfiles(user_id PK/FK, grade_level, homeroom, form_tutor_email)
StaffProfiles(user_id PK/FK, department, position)
...
Books(book_id PK, isbn UK, title, publication_year,
      publisher_id FK, genre_id FK, summary, replacement_cost)
BookAuthors(book_id FK, author_id FK, PRIMARY KEY composite)
BookCopies(copy_id PK, book_id FK, acquisition_date, condition, status)
  -- status: 'available' | 'loaned' | 'held' | 'lost' | 'retired'
...
FineRules(rule_id PK, role_id FK, grace_days, tier1_rate,
          tier2_rate, max_amount)
Fines(fine_id PK, loan_id FK, amount, accrued_date,
      paid_date NULL, status)

SchoolCalendar(day_date PK, is_open, day_type, notes)
  -- day_type: 'school_day' | 'weekend' | 'holiday' | 'exam_period'

Notifications(notification_id PK, loan_id FK NULL, recipient_id FK→Users,
              notification_type, recipient_email, sent_at, status)
  -- type: 'overdue_warning' | 'tutor_alert' | 'reservation_ready'
...
- **Email bounces:** `Notifications.status` column logs `bounced`. True bounce handling (DSN parsing) beyond IA scope.
...
- **Seasonal genre:** one genre (e.g. Garden) spikes in spring. Becomes the Criterion E "linear model can't capture seasonality" example.
...
Data generated by `generate_synthetic_data.py`. Document the methodology in dev journal; reference it (briefly) in Criterion B.
...
| # | Phase | Hours | Status |
|---|---|---|---|
| 0 | Environment setup (venv, deps, git, GitHub) | 1 | ⏳ Partial — repo exists, env ready, dependencies installed. Code wiped 2026-05-12. |
| 1 | Foundation: full schema + synthetic data seeder | 4 | ⏳ Next up |
...
- [ ] **Confirm Python version on dev machine.** Update IA draft and `requirements.txt` accordingly.
...
- [ ] **Confirm "Python 3.14" claim** in IA draft is accurate (was placeholder).
