...
CREATE TABLE Users (
    ...
    is_active       INTEGER NOT NULL DEFAULT 1 --sqlite has no bool
);

CREATE TABLE StudentProfiles (
    user_id             INTEGER PRIMARY KEY REFERENCES Users(user_id),
    year_number         INTEGER NOT NULL, --IB1 and IB2 are stored as 12/13
    homeroom            TEXT,
...
CREATE TABLE Loans (
    ...
    checkout_date   TEXT NOT NULL, -- all dates stored as text in sqlite
...
CREATE TABLE Reservations (
    ...
    priority        INTEGER NOT NULL, -- lower nr = higher (staff = 1, students = 2)
    status          TEXT NOT NULL DEFAULT 'pending'
        CHECK (status IN ('pending','ready','fulfilled','expired','cancelled')), -- 'ready' -> copy is waiting, 'fulfilled' -> collected, 'expired' -> not collected
    notified_date   TEXT,
    expires_at      TEXT -- 48h deadline    
);
...
CREATE TABLE FineRules (
    ...
    tier_1_rate REAL NOT NULL, -- initial rate 
    tier_2_rate REAL NOT NULL, -- escalated rate
    max_amount  REAL NOT NULL  -- capped fine amount
);
...
CREATE TABLE Notifications (
    ...
    status              TEXT NOT NULL DEFAULT 'sent'
        CHECK (status IN ('sent','failed','undelivered')) -- failed = sys. error, undelivered == user error
);
...
CREATE VIEW borrowing_eligibility AS -- per user borrow check: unpaid fines owed + books still out
SELECT u.user_id,
